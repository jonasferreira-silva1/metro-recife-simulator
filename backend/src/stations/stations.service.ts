...
/** Dados iniciais das 30 estações das duas linhas do Metrô do Recife */
const STATIONS_SEED = [
...
export class StationsService implements OnModuleInit {
  ...
  /**
   * Popula o banco com as estações na primeira inicialização.
   * Verifica por nome+linha para evitar duplicatas em restarts.
   */
  private async seed(): Promise<void> {
    const count = await this.stationRepository.count();

    if (count >= 30) {
      this.logger.log('Estações já populadas — seed ignorado.');
      return;
    }

    this.logger.log('Populando 30 estações da CBTU...');
    ...
    this.logger.log('Seed concluído — 30 estações inseridas.');
  }
