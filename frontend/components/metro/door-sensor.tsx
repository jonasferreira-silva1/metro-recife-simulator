...
// Máximo de tentativas antes do alerta
...
export function DoorSensor({ train }: DoorSensorProps) {
  ...
  const handleBlock = () => blockDoor(train.id);
  const handleUnblock = () => unblockDoor(train.id);

  return (
    <Card
      className={cn(
        "relative overflow-hidden transition-all",
        isBlocked && "border-destructive ring-2 ring-destructive/20"
      )}
    >
      {/* Indicador de status */}
      <div
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={cn(
                "font-medium",
                train.doorAttempts >= MAX_ATTEMPTS && "text-destructive"
              )}
            >
          ...
          <Progress
            ...
            className={cn(
              "h-1.5",
              train.doorAttempts >= MAX_ATTEMPTS && "[&>div]:bg-destructive"
            )}
          />
          {train.doorAttempts >= MAX_ATTEMPTS && (
        ...
        <div className="relative h-16 overflow-hidden rounded-lg border border-border bg-muted/30">
          ...
          <div className="absolute inset-0 flex">
            ...
            <div
              className={cn(
                "flex flex-1 items-center justify-center transition-all",
            ...
            >
              {isBlocked && (
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-[8px] uppercase text-destructive">
                    Obstrução
                  </span>
                </div>
              )}
            ...
            <div className="absolute inset-x-0 top-1/2 flex -translate-y-1/2 items-center justify-center">
              <div
                className={cn(
                  ...
                  isBlocked
                    ? "w-full animate-pulse bg-destructive"
                    : "w-1/2 bg-primary/50"
                )}
              />
            </div>
